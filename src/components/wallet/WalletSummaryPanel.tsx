    ...
    <section className={`space-y-3 ${className}`}>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        <article className="wallet-stat-card">
          ...
          <p className="wallet-stat-value">{isLoading ? '...' : formatAmount(summary.mainBalance, currency)}</p>
        </article>
        <article className="wallet-stat-card wallet-stat-card-amber">
          ...
          <p className="wallet-stat-value text-amber-300">{isLoading ? '...' : formatAmount(summary.lockedBalance, currency)}</p>
        </article>
        <article className="wallet-stat-card wallet-stat-card-emerald">
          ...
          <p className="wallet-stat-value text-emerald-300">
            {isLoading ? '...' : formatAmount(summary.withdrawableBalance, currency)}
          </p>
        </article>
      </div>
    </section>
